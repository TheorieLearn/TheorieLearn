...
def grade(data):
    if data["score"] != 1:
        ...
        if is_strictly_decreasing and has_log:
            data["feedback"]["q1"] = (
                "It seems like you're using logarithms to convert products to sums, and that your expression is strictly decreasing as $w$ increases, which turns maximization into minimization. Those are both correct approaches! However, is there a simpler expression you could use?"
            )
        elif is_strictly_decreasing:
            data["feedback"]["q1"] = (
                "It seems like your expression is strictly decreasing as $w$ increases, which is a correct approach, as it turns maximization into minimization. However, how can you address the issue that you need to turn a product of ratios into a sum?"
            )
        elif has_log:
            data["feedback"]["q1"] = (
                "You have the correct idea using logarithms to convert products into sums. However, all the relevant graph algorithms minimize sums, but we aim to maximize values. How can you address this?"
            )
        else:
            data["feedback"]["q1"] = (
