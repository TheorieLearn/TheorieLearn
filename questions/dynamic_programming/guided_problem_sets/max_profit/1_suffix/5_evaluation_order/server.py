...
def grade(data: QuestionData) -> None:
    ...
    def grade_proof(proof: list[Any]):
        ...
        if proof[0]["inner_html"] == block_dict[1]:
            return (
                0,
                "Evaluating in increasing order violates the $i+1$ dependency.",
            )
        ...
        if proof[1]["inner_html"] == block_dict[1]:
            return (
                0.33,
                "Evaluating in increasing order violates the $i+1$ dependency.",
            )
        if proof[1]["inner_html"] != block_dict[3]:
            return (
                0.66,
                "You haven't computed $MaxProfit(i)$ yet.",
            )
